...
זרימה:
  1. טוען seed (הנתונים הקיימים) מ-cities.seed.json
  2. מושך מכל מקור פעיל (data.gov.il, למ"ס, ואופציונלית nadlan)
  3. ממזג + מתייג אמינות (normalize.py)
  4. כותב output/cities.json + דוח refresh_report.md
  5. מזריק את הנתונים הטריים לתוך nadlan_analytics.html
     (מחליף את בלוק `const CITIES = [...]` — האפליקציה נשארת סטטית/אופליין)

מתוכנן להיכשל "ברכות": אם מקור נופל, שומרים על הערך הקיים ומדווחים.
"""
import json
import re
import datetime
import config
from sources import datagov, cbs, nadlan
import normalize
...
def collect_updates(cities):
    """מריץ את כל המקורות ומחזיר {city_name: {field: (value, reliability)}}."""
    ...
    try:
        ...
        report.append(f"⚠️ אוכלוסייה נכשל: {e}")
    ...
    try:
        ...
        report.append(f"⚠️ מלאי לא מכור נכשל: {e}")

    # --- nadlan.gov.il : מחיר עסקאות אמיתי (אם מופעל) ---
    if config.ENABLE_NADLAN:
        ...
        report.append("⏭️ nadlan כבוי (ENABLE_NADLAN=False) — price24 נשאר כפי שהוא")

    # --- למ"ס : מדד מחירים ארצי (למטא-נתונים/בקרת שפיות) ---
    try:
        idx = cbs.fetch_housing_price_index(6)
        report.append(f"✅ מדד מחירי דירות (למ\"ס): {len(idx)} ערכים אחרונים")
    except Exception as e:
        report.append(f"⚠️ מדד למ\"ס נכשל: {e}")
...
def run():
    ...
    # כתיבת cities.json (בלי שדות פנימיים בתוצר ה"רזה", שומרים _rel/_relScore)
    ...
    print("\n".join(report))
    print(f"\nאמינות: 🟢 {summary['high']}  🟡 {summary['mid']}  🔴 {summary['low']}")
    print(f"נכתב: {config.OUTPUT_JSON}")


def inject_into_html(cities):
    """
    מחליף את בלוק `const CITIES = [ ... ];` ב-HTML בנתונים הטריים.
    שומר רק את שדות הסכמה המקוריים (בלי _rel/_relScore) כדי לא לשבור את האפליקציה.
    """
    ...
    new_block = ("const CITIES = [\n  // עודכן אוטומטית ע\"י nadlan_pipeline בתאריך "
                 + stamp + "\n" + ",\n".join(lines) + "\n];")
    ...
    if n == 1:
        ...
        print("⚠️ לא נמצא בלוק CITIES ב-HTML — דילוג על הזרקה")


def write_report(report, summary):
    md = ["# דוח רענון נתונים — מצפן", "",
          f"תאריך: {datetime.datetime.now():%Y-%m-%d %H:%M}", "", "## מקורות", ""]
